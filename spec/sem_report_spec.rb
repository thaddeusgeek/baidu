...
describe Baidu::SEM::ReportService do
  subject{Baidu::SEM::ReportService.new($auth)}
    ...
    it "处理过期/无效id,应返回false" do
      expect{subject.getReportState({:reportId=>'7da8438bc705c9e6e747959fafce91f2'})}.to raise_error
      expect{subject.getReportState({:reportId=>'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'})}.to raise_error
    end
end
